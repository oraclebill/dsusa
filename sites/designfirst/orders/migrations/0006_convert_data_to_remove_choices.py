...
class Migration:
    ...
    def forwards(self, orm):
        for order in orm.WorkingOrder.objects.all():
            order.cabinet_material = DOOR_MATERIAL_CHOICES[order.cabinet_material or '']
            order.finish_type = FINISH_CHOICES[order.finish_type or '']
            order.save()

    def backwards(self, orm):
        def inverted_choices(choices):
            return dict([(value, key) for key, value in choices.items()])

        for order in orm.WorkingOrder.objects.all():
            order.cabinet_material = \
                   inverted_choices(DOOR_MATERIAL_CHOICES)[order.cabinet_material]
            order.finish_type = \
                   inverted_choices(FINISH_CHOICES)[order.finish_type]
            order.save()
